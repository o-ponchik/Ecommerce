...
export async function updateOrderStatus(orderId, value, property) {
  if (value === "status") {
    ...
    const response = await client
      .patch(orderId)
      .set({ isPaid: property === "true" ? true : false })
      .commit({ token: process.env.NEXT_PUBLIC_SANITY_TOKEN });
    return response;
  } else if (value === "isDelivered") {
    console.log("isDelivered from client");
    const response = await client
      .patch(orderId)
      .set({ isDelivered: property === "true" ? true : false })
      .commit({ token: process.env.NEXT_PUBLIC_SANITY_TOKEN });
    return response;
  } else if (value === "paidAt") {
    const response = await client
      .patch(orderId)
      .set({ paidAt: property })
      .commit({ token: process.env.NEXT_PUBLIC_SANITY_TOKEN });
    return response;
  } else if (value === "deliveredAt") {
    const response = await client
      .patch(orderId)
      .set({ deliveredAt: property })
      .commit({ token: process.env.NEXT_PUBLIC_SANITY_TOKEN });
    return response;
  }
}
